/*------------------------------------------------------------
 * Snooze - The "Chill" HTTP Server 💤
 *
 * Welcome to the implementation of Snooze. This isn't your average enterprise
 * HTTP server. It's designed to be tiny, simple, and perfect for testing.
 *
 * CORE PURPOSE:
 * 1. SIMULATE LATENCY:  `GET /snooze/5` waits 5 seconds. Perfect for testing
 *    timeouts and UI spinners.
 * 2. PROXY TRAFFIC:     Act as a middleman to log request metrics
 *transparently.
 * 3. BE PREDICTABLE:    Single-threaded, blocking I/O (mostly). It does one
 *    thing at a time, so you know exactly what's happening.
 *
 * HIGHLIGHTS:
 * - Single Source File:  Everything is here. No complex build chains.
 * - JSON Logging:        Structured logs ready for modern observability tools.
 * - Signal Handling:     Catches SIGINT/SIGTERM/SIGQUIT for graceful shutdown.
 *
 * Usage details are in the README.md. Implementation details are below.
 *------------------------------------------------------------*/
...
// 3. Logging function
static void log_msg(log_level_t lvl, const char *module, double exec_time,
                    const char *fmt, ...) {
  if (lvl > current_log_level)
    return;
  FILE *out = (lvl == LOG_ERROR) ? stderr : stdout;
  const char *level = (lvl == LOG_ERROR)   ? "error"
                      : (lvl == LOG_DEBUG) ? "debug"
                                           : "info";
  ...
  // Emit fixed prefix: timestamp, level, module, time (numeric)
  // time formatted as string with 4 decimal places (seconds.microseconds)
  fprintf(out,
          "{\"ts\":\"%s\",\"level\":\"%s\",\"module\":\"%s\",\"time\":\"%.4f\"",
          format_log_time(), level, module, exec_time);
...
static void parse_arguments(int argc, char *argv[], int *port,
                            const char **message, const char **proxy_host,
                            int *proxy_port) {
  int opt, env_p = 0, env_pp = 0;
  const char *env_message = NULL, *env_port = NULL;
  const char *env_loglevel = NULL;
  const char *cli_loglevel = NULL;
  const char *env_proxy_host = NULL;
  const char *env_proxy_port = NULL;
  ...
  *proxy_port = 0;
  ...
  env_proxy_host = getenv("PROXY_HOST");
  if (env_proxy_host != NULL)
    *proxy_host = env_proxy_host;

  env_proxy_port = getenv("PROXY_PORT");
  if (env_proxy_port != NULL) {
    env_pp = atoi(env_proxy_port);
    if (env_pp > 0)
      *proxy_port = env_pp;
  }
  ...
  while ((opt = getopt_long(argc, argv, "m:p:l:H:P:h", long_opts, NULL)) !=
         -1) {
    switch (opt) {
    case 'm':
      if (env_message == NULL)
    ...
    case 'h':
      ...
      printf("  -p, --port=PORT          Set the port to listen on (default: "
             "80)\n");
      printf("  -l, --loglevel=LEVEL     Set log level (error, info, debug)\n");
      printf("  -H, --proxy-host=HOST    Set the proxy target host\n");
      printf("  -P, --proxy-port=PORT    Set the proxy target port\n");
...
typedef struct {
  ...
  int proxy_port;
  double proxy_connect_time;
  double proxy_response_time;
} http_request_t;
...
static void log_request(const http_request_t *req) {
  char attrs[MAX_HEADERS_SIZE + 512];
  ...
  // Append proxy metrics if applicable
  if (req->proxy_host) {
    snprintf(
        attrs + pos, sizeof(attrs) - pos,
        ",\"proxy_host\":\"%s\",\"proxy_port\":\"%d\",\"proxy_connect_time\":"
        "\"%.4f\",\"proxy_response_time\":\"%.4f\"",
        req->proxy_host, req->proxy_port, req->proxy_connect_time,
        req->proxy_response_time);
  }
 ...
 *
 * PURPOSE:
 * Proxies the request to an upstream host:port.
 *
 * ARGS:
 * - client_fd: The socket file descriptor for the client connection.
 * - reqbuf: The raw request buffer containing headers/body received so far.
 * - host: The upstream hostname (e.g., "localhost").
 * - port: The upstream port (e.g., 8080).
 * - req: Pointer to http_request_t to populate metrics.
 *
 * METRICS:
 * - proxy_connect_time: Time taken to resolve and connect to upstream.
 * - proxy_response_time: Time taken to relay the response.
 *-----------------------------------------------------------*/
static void handle_proxy_request(int client_fd, const char *reqbuf,
  ...
  if (!server) {
    log_msg(LOG_ERROR, "proxy", 0,
            "\"op\":\"resolve\",\"host\":\"%s\",\"error\":\"not found\"", host);
    send_http_response(client_fd, "HTTP/1.1 502 Bad Gateway\r\n\r\n");
    return;
  }
  ...
  if (sockfd < 0) {
    log_msg(LOG_ERROR, "proxy", 0, "\"op\":\"socket\",\"error\":\"%s\"",
            strerror(errno));
    send_http_response(client_fd, "HTTP/1.1 500 Internal Server Error\r\n\r\n");
    return;
  }
  ...
  if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    log_msg(
        LOG_ERROR, "proxy", 0,
        "\"op\":\"connect\",\"host\":\"%s\",\"port\":\"%d\",\"error\":\"%s\"",
        host, port, strerror(errno));
    close(sockfd);
    send_http_response(client_fd, "HTTP/1.1 502 Bad Gateway\r\n\r\n");
    return;
  }
  req->proxy_connect_time = now_seconds() - t_start;

  // Send original request
  if (send_all(sockfd, reqbuf, strlen(reqbuf)) < 0) {
    log_msg(LOG_ERROR, "proxy", 0, "\"op\":\"send\",\"error\":\"%s\"",
            strerror(errno));
    close(sockfd);
    return;
  }
  ...
  double t_resp_start = now_seconds();
  ...
  req->proxy_response_time = now_seconds() - t_resp_start;
 ...
 *
 * PURPOSE:
 * Central request handler that routes requests to either:
 * 1. Proxy Handler (if proxy_host is set)
 * 2. Snooze Handler (if path starts with /snooze/N)
 * 3. Default Response Handler (all other cases)
 *-----------------------------------------------------------*/
static void handle_request(int client_fd, http_request_t *req,
                           const char *reqbuf, const char *default_message,
                           const char *proxy_host, int proxy_port) {
  if (proxy_host) {
    handle_proxy_request(client_fd, reqbuf, proxy_host, proxy_port, req);
  } else if (req->snooze_seconds > 0) {
    // Handle snooze request with precise timing
    double t1 = now_seconds();
    sleep(req->snooze_seconds);
    double t2 = now_seconds();
    double actual_sleep = t2 - t1;

    // Store exact time in req so log matches response
    req->exec_time = actual_sleep;

    char snooze_msg[128];
    snprintf(snooze_msg, sizeof(snooze_msg), "Snoozed for %.4f seconds!\n",
             actual_sleep);
 ...
 *
 * PURPOSE:
 * Single-pass HTTP header parser. Extracts key information
 * directly from the request buffer to minimize allocations.
 *
 * EXAMPLE USAGE:
 *   http_request_t req;
...
static void parse_headers(const char *reqbuf, http_request_t *req) {
  ...
  while (cur < end) {
    ...
    if (is_first_line) {
      ...
      const char *second_space =
          first_space ? memchr(first_space + 1, ' ', line_end - first_space - 1)
                      : NULL;
      ...
      if (colon && colon < line_end) {
        ...
        if (name_len == 10 && strncasecmp(cur, "User-Agent", 10) == 0) {
          ...
          json_pos +=
              snprintf(temp_buffer + json_pos, sizeof(temp_buffer) - json_pos,
                       "\"%.*s\":\"%.*s\"", (int)name_len, cur, (int)value_len,
                       value_start);

          if (json_pos >= sizeof(temp_buffer) - 100)
            break;
        }
      }
    }
  ...
  if (!req->user_agent[0])
    strncpy(req->user_agent, "unknown", sizeof(req->user_agent) - 1);
  strncpy(req->additional_headers, temp_buffer,
          sizeof(req->additional_headers) - 1);
}
...
int main(int argc, char *argv[]) {
  ...
  parse_arguments(argc, argv, &port, &message, &proxy_host, &proxy_port);
  ...
  if (server_fd < 0) {
    log_msg(LOG_ERROR, "net", now_seconds() - program_start,
            "\"op\":\"socket\",\"error\":\"%s\"", strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  if (ret < 0) {
    log_msg(LOG_ERROR, "net", now_seconds() - program_start,
            "\"op\":\"setsockopt\",\"error\":\"%s\"", strerror(errno));
    close(server_fd);
    exit(EXIT_FAILURE);
  }
  ...
  if (bind(server_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    log_msg(LOG_ERROR, "net", now_seconds() - program_start,
            "\"op\":\"bind\",\"error\":\"%s\"", strerror(errno));
    close(server_fd);
    exit(EXIT_FAILURE);
  }
  ...
  if (listen(server_fd, 10) < 0) {
    log_msg(LOG_ERROR, "net", now_seconds() - program_start,
            "\"op\":\"listen\",\"error\":\"%s\"", strerror(errno));
    close(server_fd);
    exit(EXIT_FAILURE);
  }
  ...
  if (proxy_host) {
    log_msg(LOG_INFO, "app", now_seconds() - program_start,
            "\"op\":\"proxy\",\"port\":\"%d\",\"target\":\"%s:%d\"", port,
            proxy_host, proxy_port);
  } else {
    log_msg(LOG_INFO, "app", now_seconds() - program_start,
            "\"op\":\"snooze\",\"port\":\"%d\"", port);
  }
  ...
  for (;;) {
    if (!keep_running) {
      if (!shutdown_logged) {
        ...
        const char *sig_str =
            caught_signal ? signal_name(caught_signal) : "none";
        log_msg(LOG_INFO, "app", shutdown_start - program_start,
                "\"op\":\"%s\"", sig_str);
        shutdown_logged = 1;
      }
      break;
    ...
    if (client_fd < 0) {
      /* If interrupted by a signal (e.g., Ctrl-C), continue to loop so we can
       * detect shutdown */
      if (errno == EINTR)
        continue;
      log_msg(LOG_ERROR, "net", now_seconds() - program_start,
              "\"op\":\"accept\",\"error\":\"%s\"", strerror(errno));
      continue;
    }
    ...
    while ((n = recv(client_fd, reqbuf + total, sizeof(reqbuf) - 1 - total,
                     0)) > 0) {
      total += n;
      reqbuf[total] = '\0';
      if (strstr(reqbuf, "\r\n\r\n"))
        break; // End of headers
      if (total >= sizeof(reqbuf) - 1)
        break; // Buffer full
    }
    ...
    // Handle the request (includes any snooze delay, proxying, timing updates)
    handle_request(client_fd, &request, reqbuf, message, proxy_host,
                   proxy_port);

    // Calculate total execution time if not already set by handle_request (e.g.
    // for snooze/proxy)
    gettimeofday(&end, NULL);
    if (request.exec_time == 0.0) {
      request.exec_time = (double)(end.tv_sec - start.tv_sec) +
